...
const DividerComponent = forwardRef(({ 
  ...
  const numberFormats = [
    ...
    { id: 'lower-alpha', label: 'a, b, c', convert: (num) => String.fromCharCode(96 + parseInt(num)) },
    { id: 'upper-alpha', label: 'A, B, C', convert: (num) => String.fromCharCode(64 + parseInt(num)) },
    { id: 'lower-roman', label: 'i, ii, iii', convert: (num) => toRoman(parseInt(num)).toLowerCase() },
    { id: 'upper-roman', label: 'I, II, III', convert: (num) => toRoman(parseInt(num)).toUpperCase() }
  ];
  ...
  const dividerTypes = [
    ...
    },
    {
      ...
      html_css: `<div style="width: 100%; padding: 32px 0;">
        <!-- Spacer -->
      </div>`
    }
  ];
  ...
  const handleEditClick = (block) => {
    ...
    if (block.subtype === 'continue') {
      ...
      const numberMatch = block.html_css?.match(/>([^<]+)<\/div>\s*<\/div>\s*<\/div>\s*<\/div>$/);
      const colorMatch = block.html_css?.match(/background-color:\s*([^;]+)/);
      ...
      if (/^[a-z]$/.test(extractedNumber)) {
        ...
        setNumberFormat(extractedNumber === extractedNumber.toLowerCase() ? 'lower-roman' : 'upper-roman');
      } else {
  ...
  const handleSaveChanges = () => {
    ...
    if (editingBlock.subtype === 'continue') {
      ...
      updatedHtmlCss = generateNumberedHTML(numberValue, numberFormat, numberBgColor);
    ...
    onDividerUpdate(editingBlock.id, {
      ...
      editParams: editingBlock.subtype === 'continue' 
        ? { text: continueText, color: continueColor }
        : { value: numberValue, format: numberFormat, bgColor: numberBgColor }
    });
    ...
    <>
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 group-hover:text-blue-900 text-base">{type.title}</h3>
      ...
      <Dialog open={showEditDialog} onOpenChange={handleDialogClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              Edit {editingBlock?.subtype === 'continue' ? 'Continue Button' : 'Numbered Divider'}
            </DialogTitle>
          </DialogHeader>
