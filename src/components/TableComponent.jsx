...
import { X, Plus, Save, Table, Grid, Columns, Minus, MoreHorizontal } from 'lucide-react';
...
const TableComponent = ({ 
  ...
  const tableTemplates = [
    {
      ...
      defaultData: {
        ...
        data: [['Add your content here. You can include text, lists, or images.', 'Add your content here. You can include text, lists, or images.']]
      }
    },
    {
      id: 'three_columns',
      ...
      defaultData: {
        ...
        data: [['Feature or content description', 'Feature or content description', 'Feature or content description']]
      }
    },
    {
      id: 'responsive_table',
  ...
  const handleTemplateSelect = (template) => {
    const htmlContent = generateTableHTML(template.defaultData, template.id, false);
    ...
    const tableBlock = {
      ...
      content: JSON.stringify({ ...template.defaultData, templateId: template.id, tableType: template.id }),
  ...
  const generateTableHTML = (data, templateId, isPreview = false) => {
    ...
    if (template?.type === 'layout') {
      ...
      const colClass = isPreview ? 'grid-cols-1' : 
      ...
      return `
        <div class="grid ${colClass} ${gridGap}">
          ${data.data[0].map((content, index) => `
            <div class="group relative ${padding} rounded-lg border border-gray-100 bg-gradient-to-br from-white to-gray-50 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all duration-300">
              ...
              <div class="flex items-center mb-1">
                ...
                <h3 class="font-bold ${headerSize} text-gray-900 truncate">${data.headers[index]}</h3>
              </div>
              <div class="text-gray-700 leading-tight ${textSize} line-clamp-2">${content}</div>
            </div>
          `).join('')}
        </div>
      `;
      ...
      return `
        <table class="${tableClass} divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden">
          <thead class="bg-gradient-to-r from-gray-50 to-gray-100">
            <tr>
              ${data.headers.map((header, index) => `
                <th class="${padding} text-left ${headerTextSize} font-bold text-gray-700 uppercase tracking-tight border-r border-gray-200 last:border-r-0">
            ...
            ${data.data.map((row, rowIndex) => `
              <tr class="hover:bg-gray-50 transition-colors duration-200 ${rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-25'}">
                ${row.map((cell, cellIndex) => `
                  <td class="${padding} text-gray-800 border-r border-gray-100 last:border-r-0">
                    <div class="font-medium ${textSize} truncate">${cell}</div>
                  </td>
                `).join('')}
              </tr>
            `).join('')}
  ...
  const handleSave = () => {
    ...
    const tableBlock = {
      ...
      content: JSON.stringify({ ...tableData, templateId: selectedTemplate, tableType: selectedTemplate }),
      html_css: htmlContent,
      order: editingBlock?.order || Date.now()
    };

    if (isEditing && onTableUpdate) {
      onTableUpdate(tableBlock.id, JSON.stringify({ ...tableData, templateId: selectedTemplate, tableType: selectedTemplate }), htmlContent, selectedTemplate);
    ...
    return (
      <div className="space-y-6">
        ...
        {!skipHeader && (
          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 border border-blue-200">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Edit {template.title}</h3>
        ...
        <div className="space-y-4">
          <h4 className="text-base font-semibold text-gray-900">Edit Column Content</h4>
          
          <div className="space-y-4">
            {tableData && tableData.headers ? tableData.headers.map((header, index) => (
              <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div>
                  ...
                  <textarea
                    value={(tableData.data && tableData.data[0] && tableData.data[0][index]) ? tableData.data[0][index] : ''}
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-base font-semibold text-gray-900">Edit Table Content</h4>
          ...
          <div>
            ...
            <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${tableData && tableData.headers ? tableData.headers.length : 1}, 1fr)` }}>
              {tableData && tableData.headers ? tableData.headers.map((header, index) => (
              ...
              )) : <div className="text-center text-gray-500">No headers available</div>}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {tableData && tableData.data ? tableData.data.map((row, rowIndex) => (
                <div key={rowIndex} className="flex items-center gap-2">
                  <div className="grid gap-2 flex-1" style={{ gridTemplateColumns: `repeat(${row ? row.length : 1}, 1fr)` }}>
                    {row ? row.map((cell, cellIndex) => (
                      <div key={cellIndex} className="relative">
                        ...
                        {cellIndex === 0 && tableData.headers && tableData.headers.length > 1 && (
                          <Button
                    ...
                    )) : <div className="text-center text-gray-500">No row data</div>}
                  </div>
                  {/* Remove Row Button */}
          ...
          >
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-2 group-hover:border-blue-200 transition-colors duration-300">
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="text-xs text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300">Click to add</div>
              </div>
              <div 
                className="w-full overflow-hidden max-w-full" 
                style={{ maxWidth: '100%' }}
                dangerouslySetInnerHTML={{ 
                  __html: generateTableHTML(template.defaultData, template.id, true) 
                }} 
              />
            </div>
          </div>
  ...
  if (isEditMode) {
    ...
    return (
      <Dialog open={true} onOpenChange={onClose}>
        <DialogContent className={`max-w-4xl max-h-[90vh] overflow-hidden flex flex-col transition-all duration-300 ease-out transform ${
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-6">
              {template?.type === 'layout' ? renderColumnEditor(template, true) : renderDataTableEditor(template, true)}
            </div>
          </div>
        </DialogContent>
      </Dialog>
  ...
  return (
    <div className={`fixed inset-0 bg-black flex items-center justify-start z-50 transition-all duration-300 ease-out ${
      isVisible ? 'bg-opacity-50' : 'bg-opacity-0'
    }`}>
      <div className={`bg-white h-full w-[480px] shadow-2xl flex flex-col transform transition-all duration-500 ease-out ${
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-purple-50">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">Choose from beautiful designs</p>
            </div>
          </div>
          <Button
            onClick={onClose}
