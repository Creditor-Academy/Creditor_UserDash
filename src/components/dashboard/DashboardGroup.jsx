                ...
                groupList.map(async (g) => {
                  try {
                    const [postsRes, annRes] = await Promise.all([
                      getGroupPosts(g.id).catch(() => ({
                        success: false,
                        data: [],
                      })),
                      getAnnouncements(g.id).catch(() => ({
                        success: false,
                        data: [],
                      })),
                    ]);
                    ...
                    const mediaFromPosts = Array.isArray(posts)
                      ? posts.filter((p) => Boolean(p.media_url || p.media))
                          .length
                      : 0;
                    const mediaFromAnnouncements = Array.isArray(announcements)
                      ? announcements.filter((a) =>
                          Boolean(a.media || a.media_url),
                        ).length
                      : 0;
                    ...
                    const mediaUrls = [
                      ...(Array.isArray(posts)
                        ? posts
                            .map((p) => p.media_url || p.media)
                            .filter(isImageUrl)
                        : []),
                      ...(Array.isArray(announcements)
                        ? announcements
                            .map((a) => a.media_url || a.media)
                            .filter(isImageUrl)
                        : []),
                    ].slice(0, 3);
                    ...
                    return [
                      g.id,
                      { posts: 0, announcements: 0, media: 0, mediaUrls: [] },
                    ];
                  }
                }),
              );
