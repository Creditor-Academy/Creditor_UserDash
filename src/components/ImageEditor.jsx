...
import { RotateCcw, RotateCw, Crop, ZoomIn, ZoomOut, X, Check } from 'lucide-react';
...
const ImageEditor = ({ 
  ...
  const drawCropOverlay = useCallback((ctx, imgX, imgY, imgWidth, imgHeight) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
  ...
  const getHandleAt = useCallback((mouseX, mouseY) => {
    ...
    if (Math.abs(mouseX - (canvasArea.x + canvasArea.width)) < tolerance && Math.abs(mouseY - canvasArea.y) < tolerance) {
      return 'resize-ne'; // Top-right
    }
    if (Math.abs(mouseX - canvasArea.x) < tolerance && Math.abs(mouseY - (canvasArea.y + canvasArea.height)) < tolerance) {
    ...
    if (mouseX >= canvasArea.x && mouseX <= canvasArea.x + canvasArea.width &&
        mouseY >= canvasArea.y && mouseY <= canvasArea.y + canvasArea.height) {
      return 'move';
    }
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    if (dragType === 'move') {
      ...
      setCropArea(prev => {
        const newX = Math.max(0, Math.min(image.width - prev.width, prev.x + deltaX * scaleX));
        const newY = Math.max(0, Math.min(image.height - prev.height, prev.y + deltaY * scaleY));
        console.log('Moving crop area:', { x: newX, y: newY, width: prev.width, height: prev.height });
        return {
          x: newX,
          y: newY,
          width: prev.width,
      ...
      setCropArea(prev => {
        ...
        console.log('Resizing crop area:', { x: newX, y: newY, width: newWidth, height: newHeight });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      finalCtx.drawImage(
        image,
        cropArea.x, cropArea.y, cropArea.width, cropArea.height, // Source: crop area from original image
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the area you want to KEEP. Everything outside the green rectangle will be CUT OUT (cropped away).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex gap-6 h-[500px]">
          ...
          <div className="w-80 space-y-6">
            ...
            <div className="pt-4 border-t">
              <h4 className="text-sm font-medium text-gray-700 mb-3">Crop Area (What You'll Keep)</h4>
