...
const BlockEditorTab = ({ 
  ...
  const getDefaultContent = (type) => {
    switch (type) {
      ...
      case 'youtube':
        return { url: '', title: 'YouTube Video', width: '100%', height: '315' };
      case 'link':
        return { url: '', text: 'Link text', description: '', openInNewTab: true };
      case 'pdf':
      ...
      case 'scorm':
        return { packageUrl: '', title: 'SCORM Package', width: '100%', height: '600px' };
      default:
        return '';
    }
  };
  ...
  const confirmDelete = () => {
    if (blockToDelete) {
      setContentBlocks(prev => ({
        ...prev,
        [editingLessonId]: prev[editingLessonId].filter(block => block.id !== blockToDelete.id)
      }));
  ...
  const bulkDeleteBlocks = () => {
    ...
    setContentBlocks(prev => ({
      ...prev,
      [editingLessonId]: prev[editingLessonId].filter(block => !selectedBlocks.has(block.id))
    }));
    ...
    switch (block.type) {
      case 'text':
        return (
          <div className="p-4 space-y-3">
            <div>
              <Label htmlFor={`text-${block.id}`} className="text-sm font-medium text-gray-700">
                Text Content
              </Label>
              <Textarea
                id={`text-${block.id}`}
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => updateContentBlock(block.id, block.content, { bold: checked })}
              />
              <Label htmlFor={`bold-${block.id}`} className="text-sm">Bold text</Label>
            </div>
          </div>
        );
      case 'heading':
        return (
          <div className="p-4 space-y-3">
            <div>
              <Label htmlFor={`heading-${block.id}`} className="text-sm font-medium text-gray-700">
                Heading Text
              </Label>
              <Input
                id={`heading-${block.id}`}
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <Label htmlFor={`level-${block.id}`} className="text-sm">Level:</Label>
                <select
                  id={`level-${block.id}`}
                  value={block.settings?.level || 'h2'}
                  onChange={(e) => updateContentBlock(block.id, block.content, { level: e.target.value })}
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => updateContentBlock(block.id, block.content, { centered: checked })}
                />
                <Label htmlFor={`center-${block.id}`} className="text-sm">Center align</Label>
              </div>
            </div>
          </div>
        );
        ...
        return (
          <div className="p-4">
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg">
              <textarea
                value={typeof block.content === 'string' ? block.content.replace(/<[^>]*>/g, '') : ''}
                onChange={(e) => updateContentBlock(block.id, `<div class="bg-blue-50 border-l-4 border-blue-400 p-4"><p><strong>Important:</strong> ${e.target.value}</p></div>`)}
        ...
        return (
          <div className="p-4">
            <div className="border-l-4 border-gray-400 bg-gray-50 p-4 rounded-r-lg">
              <textarea
                value={typeof block.content === 'string' ? block.content.replace(/<[^>]*>/g, '').replace(/"/g, '') : ''}
                onChange={(e) => updateContentBlock(block.id, `<blockquote class="border-l-4 border-gray-300 pl-4 italic text-gray-700">"${e.target.value}"</blockquote>`)}
          ...
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div>
                  <Label htmlFor={`img-url-${block.id}`} className="text-sm font-medium text-gray-700">
                    Image URL
                  </Label>
                  <Input
                    id={`img-url-${block.id}`}
                ...
                <div>
                  <Label htmlFor={`img-alt-${block.id}`} className="text-sm font-medium text-gray-700">
                ...
                <div>
                  <Label htmlFor={`img-caption-${block.id}`} className="text-sm font-medium text-gray-700">
                    Caption (optional)
                  </Label>
                  <Input
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <Label htmlFor={`img-width-${block.id}`} className="text-sm">Width:</Label>
                    <select
                      id={`img-width-${block.id}`}
                      value={block.content?.width || '100%'}
                      onChange={(e) => updateContentBlock(block.id, { ...block.content, width: e.target.value })}
                      className="px-2 py-1 border border-gray-300 rounded text-sm"
                    >
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateContentBlock(block.id, block.content, { centered: checked })}
                    />
                    <Label htmlFor={`img-center-${block.id}`} className="text-sm">Center image</Label>
                  </div>
                </div>
              </div>
        ...
        return (
          <div className="p-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div>
                  <Label htmlFor={`video-url-${block.id}`} className="text-sm font-medium text-gray-700">
                  ...
                  <Input
                    ...
                    value={block.content?.url || ''}
                    onChange={(e) => updateContentBlock(block.id, { ...block.content, url: e.target.value })}
                    placeholder={block.type === 'youtube' ? 'https://youtube.com/watch?v=...' : 'https://example.com/video.mp4'}
                    className="mt-1"
                  />
                </div>
                <div>
                  <Label htmlFor={`video-title-${block.id}`} className="text-sm font-medium text-gray-700">
                    Video Title
                  </Label>
                  <Input
                ...
                <div>
                  <Label htmlFor={`video-desc-${block.id}`} className="text-sm font-medium text-gray-700">
                ...
                <div className="flex items-center space-x-4">
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) => updateContentBlock(block.id, { ...block.content, autoplay: checked })}
                    />
                    <Label htmlFor={`video-autoplay-${block.id}`} className="text-sm">Autoplay</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Switch
                      id={`video-controls-${block.id}`}
                      checked={block.content?.controls !== false}
                      onCheckedChange={(checked) => updateContentBlock(block.id, { ...block.content, controls: checked })}
                    />
                    <Label htmlFor={`video-controls-${block.id}`} className="text-sm">Show controls</Label>
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-center">
                  ...
                  <div className="w-full">
                    {block.type === 'youtube' ? (
                      <div className="aspect-video bg-gray-100 rounded border flex items-center justify-center">
                        ...
                        <span className="ml-2 text-gray-500">YouTube Preview</span>
                      </div>
                    ) : (
                      <div className="aspect-video bg-gray-100 rounded border flex items-center justify-center">
                    ...
                    {block.content.title && (
                      <p className="text-sm font-medium text-gray-700 mt-2">{block.content.title}</p>
                    )}
                  </div>
                ) : (
      ...
      case 'list':
        return (
          <div className="p-4 space-y-3">
            <div>
              <Label htmlFor={`list-${block.id}`} className="text-sm font-medium text-gray-700">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <Label htmlFor={`list-type-${block.id}`} className="text-sm">Type:</Label>
                <select
                  id={`list-type-${block.id}`}
                  value={block.settings?.listType || 'bulleted'}
                  onChange={(e) => updateContentBlock(block.id, block.content, { listType: e.target.value })}
                  className="px-2 py-1 border border-gray-300 rounded text-sm"
                >
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="border rounded-lg p-4 bg-gray-50">
              <div className="text-center text-gray-500">
                ...
                <p className="text-xs text-gray-400 mt-1">For now, you can add simple tables using HTML</p>
              </div>
            </div>
          </div>
        );
      case 'link':
        return (
          <div className="p-4 space-y-3">
            <div>
              <Label htmlFor={`link-url-${block.id}`} className="text-sm font-medium text-gray-700">
                Link URL
              </Label>
              <Input
            ...
            <div>
              <Label htmlFor={`link-text-${block.id}`} className="text-sm font-medium text-gray-700">
                Link Text
              </Label>
              <Input
            ...
            <div>
              <Label htmlFor={`link-desc-${block.id}`} className="text-sm font-medium text-gray-700">
                Description (optional)
              </Label>
              <Textarea
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => updateContentBlock(block.id, { ...block.content, openInNewTab: checked })}
              />
              <Label htmlFor={`link-newtab-${block.id}`} className="text-sm">Open in new tab</Label>
            </div>
          </div>
        );
      default:
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                {contentBlockTypes.filter(block => ['text', 'heading', 'statement', 'quote', 'list'].includes(block.id)).map((blockType) => (
                  <Card
                    key={blockType.id}
                  ...
                  >
                    <CardContent className="flex items-center p-3">
                      ...
                      <div>
                        <h4 className="text-sm font-medium text-gray-800">{blockType.title}</h4>
            ...
            {/* Media Blocks */}
            <div>
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Media</h4>
              <div className="space-y-2">
                {contentBlockTypes.filter(block => ['image', 'video', 'youtube', 'audio'].includes(block.id)).map((blockType) => (
                  <Card
                    key={blockType.id}
                    className="cursor-pointer hover:shadow-md transition-all duration-200 border border-gray-200 hover:border-purple-200 hover:bg-purple-50"
                    onClick={() => addContentBlock(blockType)}
            ...
            {/* Interactive Blocks */}
            <div>
              <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Interactive</h4>
              <div className="space-y-2">
                {contentBlockTypes.filter(block => ['link', 'table', 'checklist', 'pdf', 'scorm'].includes(block.id)).map((blockType) => (
                  <Card
                    key={blockType.id}
                    className="cursor-pointer hover:shadow-md transition-all duration-200 border border-gray-200 hover:border-purple-200 hover:bg-purple-50"
                    onClick={() => addContentBlock(blockType)}
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              {currentLesson ? (
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">Select a Lesson to Edit</h3>
                  <div className="mt-2">
                    <select
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-sm font-medium text-blue-900">
                  {selectedBlocks.size} of {currentBlocks.length} blocks selected
                </span>
                <Button
          ...
          {!editingLessonId ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                Select a lesson from the dropdown above or from the outline/lessons tab to start editing its content blocks.
              </p>
              {lessons.length > 0 && (
                <Button
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-600">
              Are you sure you want to delete this {blockToDelete?.type} block? This action cannot be undone.
            </DialogDescription>
          </DialogHeader>
