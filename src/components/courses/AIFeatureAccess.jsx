...
const AI_FEATURES = {
  IMAGE_GENERATION: {
    ...
    enabled: true,
    models: ['dreamlike-art/dreamlike-photoreal-2.0', 'prompthero/openjourney-v4'],
...
export const AIFeatureAccessProvider = ({ children }) => {
  ...
  const hasAccess = (featureId) => {
    ...
    switch (userPlan) {
      ...
      case 'basic':
        return todayUsage < (feature.maxGenerationsPerDay || feature.maxSummarizationsPerDay || feature.maxQueriesPerDay || feature.maxOutlinesPerDay) / 2;
      case 'free':
  ...
  const getUsageLimit = (featureId) => {
    ...
    const baseLimit = feature.maxGenerationsPerDay || feature.maxSummarizationsPerDay || feature.maxQueriesPerDay || feature.maxOutlinesPerDay || 100;
...
export const useAIFeatureAccess = () => {
  ...
  if (!context) {
    throw new Error('useAIFeatureAccess must be used within AIFeatureAccessProvider');
  }
  return context;
};
...
export const withAIFeatureAccess = (WrappedComponent, requiredFeature) => {
  return function AIFeatureProtectedComponent(props) {
    const { hasAccess, trackUsage, getTodayUsage, getUsageLimit } = useAIFeatureAccess();
    ...
    if (!canAccess) {
      return (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-yellow-800 mb-2">Feature Access Limited</h3>
          <p className="text-yellow-700 mb-4">
            You've reached your daily limit for this AI feature ({todayUsage}/{usageLimit}).
          </p>
