        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <p className="text-sm sm:text-base text-gray-600 leading-relaxed max-w-2xl mb-4">
              TCommerce is our suite of proven selling and payments features, specifically designed for learning businesses like yours.{' '}
              <span className="font-semibold text-gray-900">Customers using TCommerce sell more, seeing up to 31% larger average transaction sizes.</span>
            </p>
            <p className="text-sm sm:text-base text-gray-600 leading-relaxed max-w-2xl mb-6">
              Explore powerful, friction-free selling features to see just how far your business can grow.
            </p>
            ...
            <div className="relative w-full flex items-center justify-center">
              ...
              <div className="relative w-full max-w-[660px] aspect-[4/3] bg-[#0b3a5a] flex items-center justify-center overflow-hidden shadow-2xl rounded-none">
                ...
                <div className="absolute inset-0">
                  <svg className="w-full h-full" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice">
                    {[...Array(64)].map((_, i) => {
                      ...
                      return (
                        <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#133b5f" strokeWidth="1.2" opacity="0.35" />
                      );
                    })}
                  </svg>
                </div>
                ...
                <div className="relative z-10 bg-white rounded-sm shadow-2xl w-[86%] ml-auto mr-6 p-5">
                  <div className="text-xl sm:text-2xl font-semibold text-gray-900 mb-3">Subscription Count</div>
                  {/* Simple bar chart */}
                  <svg viewBox="0 0 800 240" className="w-full h-48">
                    ...
                    {Array.from({ length: 10 }).map((_, idx) => {
                      ...
                      const x = 80 + idx * 55;
                      const y = 220 - height;
                      const color = idx === 9 ? '#c24f0a' : `hsl(24, 80%, ${80 - idx * 4}%)`;
                      return <rect key={idx} x={x} y={y} width="40" height={height} rx="4" fill={color} />;
                    })}
                    {/* x-axis labels (approx) */}
                    {['January','February','March','April','May','June'].map((m, i) => (
                      <text key={m} x={95 + i * 165} y={225} fontSize="11" fill="#777">{m}</text>
                    ))}
                  </svg>
                </div>

                {/* Pink Revenue Card overlay */}
                <div className="absolute left-6 bottom-6 bg-[#e9a9b9] shadow-2xl px-6 py-5 sm:px-8 sm:py-6 z-20" style={{ transform: 'translateY(12%)' }}>
                  <div className="text-3xl sm:text-4xl font-serif text-gray-900 mb-1">$246,301</div>
                  <div className="text-base sm:text-lg font-medium text-gray-900">Total Revenue</div>
                </div>
              </div>
            </div>
          </motion.div>
