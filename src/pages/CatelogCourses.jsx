...
const CatelogCourses = () => {
  ...
  /**
   * Purchase Context from My Courses page
   *
   ...
   *
   * This is passed via route state when navigating from My Courses page.
  ...
  /**
   * Check if a course is unlocked based on purchase context
   *
   * Logic:
   * - If purchaseType is 'CATALOG' → All courses unlocked
   * - If purchaseType is 'COURSE' → Only courses in unlockedItems.courseIds are unlocked
   * - If purchaseType is 'MODULE' → Only courses that contain purchased modules are unlocked
   * - If no purchase context → Fall back to checking enrolled courses
   *
   * @param {Object} course - Course object
   * @returns {boolean} True if course is unlocked/accessible
   */
  const isCourseUnlocked = course => {
    // If user is enrolled (from regular enrollment), always unlocked
    if (accessibleCourseIds.includes(course.id)) {
      return true;
    }

    // If no purchase context, fall back to old logic
    if (!purchaseContext) {
      return false;
    }
    ...
    // CATALOG purchase → All courses unlocked
    if (purchaseType === 'CATALOG') {
      return true;
    }

    // COURSE purchase → Only specific courses unlocked
    if (purchaseType === 'COURSE') {
      return unlockedItems.courseIds.includes(course.id);
    }

    // MODULE purchase → Only courses containing purchased modules are unlocked
   ...
   *
   * This determines if the "Buy Course" button should be shown.
   * If course is already unlocked (via purchase context or enrollment), hide buy button.
   *
   * @param {Object} course - Course object
   * @returns {boolean} True if buy button should be shown
   */
  const canBuyCourse = course => {
    // If course is already unlocked, don't show buy button
          ...
          {filteredCourses.length === 0 ? (
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredCourses.map((course, idx) => {
                 ...
                 *
                 * Uses purchase context if available, otherwise falls back to enrolled courses.
                 * This determines if the course should be unlocked or locked.
                 */
                const isAccessible = isCourseUnlocked(course);
                const cardContent = (
                  <div
                    ...
                    <div className="p-6 pt-0">
                      {isAccessible ? (
                        ...
                        <div className="w-full">
                          {/* 
                             Course is locked - Show "View Course" button to allow preview
                             but course content will be locked in CourseView.jsx
                           */}
                          <Link
                            to={`/dashboard/courses/${course.id}`}
                            state={{
                              purchaseContext: purchaseContext, // Pass purchase context to CourseView
                            }}
                            className="w-full"
                          >
                            <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white">
                          ...
                          {/* 
                             Show message if user has purchased modules from this catalog
                             but this specific course is locked
                           */}
                          {purchaseContext &&
                            purchaseContext.purchaseType === 'MODULE' &&
                            !isAccessible && (
                              <p className="text-xs text-gray-500 mt-2 text-center">
                                This course is locked. You have purchased
                                modules from other courses in this catalog.
                              </p>
                            )}
                        </div>
                      )}
