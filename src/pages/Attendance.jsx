...
const Attendance = () => {
  ...
  useEffect(() => {
    ...
    let filteredData;
    if (selectedMonth === "overall" || typeof selectedMonth !== 'object' || !(selectedMonth instanceof Date)) {
      ...
      try {
        ...
        filteredData = allAttendanceData.filter((record) => {
          ...
          return isWithinInterval(record.dateObj, { start: monthStart, end: monthEnd });
        });
      } catch (error) {
    ...
    allAttendanceData.forEach(record => {
      if (record.dateObj && !isNaN(record.dateObj.getTime())) {
        try {
          ...
          console.warn('Error formatting date for month key:', record.dateObj, error);
        }
      }
    });
  ...
  const getCurrentMonthIndex = () => {
    if (selectedMonth === "overall" || typeof selectedMonth !== 'object' || !(selectedMonth instanceof Date) || availableMonths.length === 0) {
      return -1;
    }
    try {
      return availableMonths.findIndex(month => 
        format(startOfMonth(month), 'yyyy-MM') === format(startOfMonth(selectedMonth), 'yyyy-MM')
      );
    } catch (error) {
  ...
  const canGoPrevious = currentMonthIndex > -1 && currentMonthIndex < availableMonths.length - 1;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Button 
              ...
              disabled={availableMonths.length === 0 || selectedMonth === "overall" || !(selectedMonth instanceof Date) || !canGoPrevious}
            >
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Select 
              value={selectedMonth === "overall" ? "overall" : (selectedMonth instanceof Date ? format(startOfMonth(selectedMonth), 'yyyy-MM') : "overall")} 
              onValueChange={(value) => {
                if (value === "overall") {
                  setSelectedMonth("overall");
                } else {
            ...
            >
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder="Select month">
                  {selectedMonth === "overall" ? "Overall" : (selectedMonth instanceof Date ? format(selectedMonth, 'MMMM yyyy') : "Overall")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
            ...
            <Button 
              ...
              disabled={availableMonths.length === 0 || selectedMonth === "overall" || !(selectedMonth instanceof Date) || !canGoNext}
            >
                ...
                <table className="w-full caption-bottom text-sm">
                  <thead className="[&_tr]:border-b sticky top-0 z-10 bg-white shadow-sm">
                    <tr className="border-b border-gray-200">
                      <th className="w-[120px] h-12 px-4 text-left align-middle font-semibold bg-white">Date</th>
                      <th className="h-12 px-4 text-left align-middle font-semibold bg-white">Course</th>
                      <th className="h-12 px-4 text-left align-middle font-semibold bg-white">Class/Event</th>
                      <th className="h-12 px-4 text-left align-middle font-semibold bg-white">Time</th>
                      <th className="h-12 px-4 text-left align-middle font-semibold bg-white">Status</th>
                    </tr>
                  </thead>
                  <tbody className="[&_tr:last-child]:border-0">
                    {tableData.length > 0 ? (
                      tableData.map((row, index) => (
                        <tr key={row.id || `attendance-${index}-${row.eventId || row.date || index}`} className="border-b transition-colors hover:bg-muted/50">
                          <td className="p-4 align-middle font-medium w-[120px]">{row.date}</td>
                          <td className="p-4 align-middle font-medium">{row.courseTitle}</td>
                          <td className="p-4 align-middle font-medium">{row.className}</td>
                          <td className="p-4 align-middle text-muted-foreground">{row.time}</td>
                          <td className="p-4 align-middle">{getStatusBadge(row.status)}</td>
                        </tr>
                      ))
                    ) : (
