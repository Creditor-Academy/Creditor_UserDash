  ...
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      handleEditImage: (blockId) => {
        ...
        // Extract generation type from block
        const blockGenerationType =
          block.details?.generationType ||
          block.metadata?.generationType ||
          block.generationType ||
          null;
        setGenerationType(blockGenerationType);
    ...
    const handleImageTemplateSelect = (template) => {
      ...
      const newBlock = {
        ...
        // Order will be calculated by parent LessonBuilder component
        details: {
          image_url: imageUrl,
          caption: imageText,
    ...
    const handleImageFileUpload = async (blockId, file, retryCount = 0) => {
      ...
      try {
        ...
        if (uploadResult.success && uploadResult.imageUrl) {
          ...
          // CRITICAL: Update form state to show the uploaded image in the dialog
    ...
    const generateImageBlockHtml = (block) => {
      ...
      if (layout === "side-by-side") {
        // Image Right, Text Left = alignment 'right' = flex-row-reverse
    ...
    const handleAddImage = async () => {
      ...
      try {
        ...
        // Wait a brief moment to ensure state updates and then close
        setTimeout(() => {
          setMainImageUploading(false);
          handleImageDialogClose();
        }, 300);
      } catch (error) {
        devLogger.error("Error in handleAddImage:", error);
      ...
      <>
        ...
        <Dialog open={showImageDialog} onOpenChange={handleImageDialogClose}>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {currentBlock ? "Edit Image" : "Add Image"}
              </DialogTitle>
              <DialogDescription>
                {currentBlock
                  ? "Update the image details, upload a new image, or choose from the gallery."
                  : "Add a new image to your lesson. You can upload an image or choose from the gallery."}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              {/* Text on/with Image - Hidden for full width standalone */}
              {!(
                standaloneImageAlignment === "full" &&
                !currentBlock?.layout?.includes("side") &&
                !currentBlock?.layout?.includes("overlay")
              ) && (
                <div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`p-3 border-2 rounded-lg text-left transition-all ${
                        currentBlock?.templateType === type.id ||
                        currentBlock?.layout === type.id
                ...
                {imagePreview && (
                  <div className="mt-4 space-y-2">
                    <div className="flex items-start justify-between">
                      ...
                      {/* Only show crop button for local files, not for AI-generated S3 images */}
                      {imageFile && !imagePreview.includes("s3") && (
                        <Button
                          type="button"
                          variant="outline"
                          size="sm"
                          onClick={() => {
                            setImageToEdit(imageFile || imagePreview);
                            setImageEditorTitle("Crop Image");
                            setShowImageEditor(true);
                          }}
              ...
              >
                {mainImageUploading ? (
                  <>
                    ...
                    {currentBlock ? "Updating..." : "Uploading..."}
                  </>
                ) : currentBlock ? (
                  "Update Image"
                ) : (
