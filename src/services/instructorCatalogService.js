...
export async function fetchAllCatalogs() {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/catalog/getallcatalogs`,
      {
        method: 'GET',
    ...
    const localCatalogs = JSON.parse(
      localStorage.getItem('localCatalogs') || '[]'
    );

    // Filter out local catalogs that might duplicate backend catalogs
    const backendCatalogIds = catalogs.map(cat => cat.id);
    const uniqueLocalCatalogs = localCatalogs.filter(
      localCat =>
    ...
    if (uniqueLocalCatalogs.length !== localCatalogs.length) {
      localStorage.setItem(
        'localCatalogs',
        JSON.stringify(uniqueLocalCatalogs)
      );
    }
...
export async function createCatalog(catalogData) {
  try {
    ...
    if (
      sanitizedData.description.length < 1 ||
      sanitizedData.description.length > 1000
    ) {
      throw new Error(
        'Catalog description must be between 1 and 1000 characters'
      );
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        response = await fetch(
          `${import.meta.env.VITE_API_BASE_URL}/api/catalog/createcatalog`,
          {
            method: 'POST',
            headers: headers,
            credentials: 'include',
    ...
    if (
      ...
      const localCatalog = {
        ...
        ...(sanitizedData.price && { price: sanitizedData.price }),
      ...
      const existingCatalogs = JSON.parse(
        localStorage.getItem('localCatalogs') || '[]'
      );
...
export async function updateCatalog(catalogId, catalogData) {
  try {
    ...
    // Try backend update
    const response = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/catalog/${catalogId}/updatecatalog`,
      {
        method: 'PUT',
        headers: getAuthHeaders(),
        credentials: 'include',
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        const catalogIndex = localCatalogs.findIndex(
        ...
        if (catalogIndex !== -1) {
          ...
          const existingCatalog = localCatalogs.find(
            cat =>
          ...
          if (!existingCatalog) {
            ...
            const existingIndex = localCatalogs.findIndex(
              cat => cat.id === existingCatalog.id
            );
        ...
        return {
          success: true,
          message:
            'Catalog updated locally (permission denied for backend update)',
          data: localCatalogs.find(
            cat => cat.id === catalogId || cat.originalId === catalogId
          ),
      ...
      if (response.status === 500) {
        ...
        return {
          success: true,
          message:
            'Catalog updated locally (server error prevented backend update)',
          data: localCatalogs.find(
            cat => cat.id === catalogId || cat.originalId === catalogId
          ),
...
export async function deleteCatalog(catalogId) {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/catalog/${catalogId}/deletecatalog`,
      {
        method: 'DELETE',
        headers: getAuthHeaders(),
        credentials: 'include',
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        const updatedLocalCatalogs = localCatalogs.filter(
          cat => cat.id !== catalogId && cat.originalId !== catalogId
        );
        localStorage.setItem(
          'localCatalogs',
          JSON.stringify(updatedLocalCatalogs)
        );

        return {
          success: true,
          message:
            'Catalog removed locally (permission denied for backend deletion)',
          warning:
            'Catalog removed from local storage due to permission restrictions',
        };
      }
...
export async function addCoursesToCatalog(catalogId, courseIds) {
  try {
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_BASE_URL}/api/catalog/${catalogId}/addcourses`,
      {
        method: 'POST',
        headers: getAuthHeaders(),
        credentials: 'include',
    ...
    if (
      ...
      console.warn(
        `Backend returned ${response.status} error, handling locally`
      );
  ...
  try {
    ...
    if (catalogId.startsWith('local-')) {
      ...
      if (catalogIndex !== -1) {
        ...
        localCatalogs[catalogIndex].courses = localCatalogs[
          catalogIndex
        ].courses.filter(courseId => !courseIds.includes(courseId));
        localStorage.setItem('localCatalogs', JSON.stringify(localCatalogs));
  ...
  try {
    ...
    if (courses.length > 0 && courses[0]) {
      ...
      const hasMinimalData =
        ...
        !firstCourse.price &&
        !firstCourse.estimated_duration;

      // Check if we have junction table data (course_id field)
      const hasJunctionData =
        firstCourse.course_id || (firstCourse.course && firstCourse.course.id);
      ...
      if (hasMinimalData || hasJunctionData) {
        ...
        try {
          ...
          if (allCoursesResponse.ok) {
            ...
            const fullCourses = courseIds
              .map(courseId => {
                const foundCourse = publishedCourses.find(
                  c => c.id === courseId
                );
                if (foundCourse) {
                  return foundCourse;
