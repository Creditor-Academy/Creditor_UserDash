...
class AIServiceRouter {
          ...
          switch (provider) {
            case 'openai':
              if (this.validateApiKey('openai')) {
                result = await this.generateTextWithOpenAI(
                  sanitizedPrompt,
                  sanitizedOptions
                );
              } else {
    ...
    try {
      ...
      for (const provider of providers) {
        try {
          ...
          switch (provider) {
            case 'stability':
              if (this.validateApiKey('stability')) {
                result = await this.generateImageWithStability(
                  sanitizedPrompt,
                  sanitizedOptions
                );
              } else {
            ...
            case 'openai':
              if (this.validateApiKey('openai')) {
                result = await this.generateImageWithDalle(
                  sanitizedPrompt,
                  sanitizedOptions
                );
              } else {
          ...
          errors.push(`${provider}: ${errorMsg}`);
          console.warn(
            `⚠️ Image generation failed with ${provider}:`,
            errorMsg
          );
        ...
        switch (provider) {
          case 'assemblyai':
            if (this.apiKeys.assemblyai) {
              return await this.speechToTextWithAssemblyAI(
                audioBuffer,
                options
              );
            }
  ...
  async speechToTextWithAssemblyAI(audioBuffer, options = {}) {
    ...
    if (!transcribeResponse.ok) {
      throw new Error(
        `AssemblyAI transcribe error: ${transcribeResponse.status}`
      );
    }
  ...
  validateImageInput(prompt, options) {
    ...
    if (prompt.length > 1000) {
      return {
        valid: false,
        error: 'Image prompt too long (max 1,000 characters)',
      };
    }
  ...
  sanitizeImageOptions(options) {
    ...
    if (sanitized.width)
      sanitized.width = Math.min(Math.max(sanitized.width, 256), 1024);
    if (sanitized.height)
      sanitized.height = Math.min(Math.max(sanitized.height, 256), 1024);
    if (sanitized.steps)
      sanitized.steps = Math.min(Math.max(sanitized.steps, 10), 50);
