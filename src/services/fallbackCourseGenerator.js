...
class FallbackCourseGenerator {
  constructor() {
    ...
    this.subjectKeywords = {
      technology: ['programming', 'coding', 'software', 'web', 'app', 'development', 'javascript', 'python', 'react', 'node', 'api', 'database', 'ai', 'machine learning', 'data', 'cloud', 'devops', 'cybersecurity'],
      business: ['management', 'marketing', 'finance', 'accounting', 'sales', 'strategy', 'leadership', 'entrepreneurship', 'startup', 'business', 'economics', 'project management'],
      academic: ['science', 'mathematics', 'history', 'literature', 'philosophy', 'psychology', 'sociology', 'research', 'theory', 'analysis', 'study'],
      creative: ['design', 'art', 'photography', 'writing', 'music', 'video', 'animation', 'creative', 'artistic', 'visual', 'audio', 'media']
    };
  }
  ...
  generateCourseOutline(courseData) {
    try {
      console.log('📚 Generating fallback course outline for:', courseData.title);

      const category = this.determineCourseCategory(courseData.title, courseData.subject || '');
      ...
      const customizedModules = template.modules.map((module, index) => {
        return {
          title: this.customizeModuleTitle(module.title, courseData.title, index + 1),
          description: this.generateModuleDescription(module.title, courseData.title),
          lessons: module.lessons.map((lesson, lessonIndex) => ({
            title: this.customizeLessonTitle(lesson, courseData.title, index + 1, lessonIndex + 1),
            description: this.generateLessonDescription(lesson, courseData.title),
      ...
      return {
        ...
        data: {
          ...
          totalLessons: customizedModules.reduce((total, module) => total + module.lessons.length, 0),
  ...
  customizeModuleTitle(templateTitle, courseTitle, moduleNumber) {
    ...
    return customizations[templateTitle] || `${subject} - Module ${moduleNumber}`;
  }
  ...
  generateModuleDescription(moduleTitle, courseTitle) {
    ...
    return descriptions[moduleTitle] || `Comprehensive coverage of ${subject} concepts and practical applications in this essential module.`;
  }
  ...
  estimateLessonDuration(lessonIndex) {
    const durations = ['25 minutes', '30 minutes', '35 minutes', '40 minutes', '45 minutes'];
