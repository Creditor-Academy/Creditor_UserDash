...
api.interceptors.response.use(
  (response) => {
    ...
    // Broadcast org refresh when uploads finish so storage/tokens can update
    try {
      if (typeof window !== "undefined") {
        const url = response.config?.url || "";
        if (
          url.includes("/api/resource/upload-resource") ||
          url.includes("/api/ai/upload-resource")
        ) {
          window.dispatchEvent(
            new CustomEvent("org:refreshSingleOrg", {
              detail: {
      ...
      console.debug("[API] Unable to dispatch org refresh event:", eventError);
    }
