...
export async function generateAndUploadCourseImage(prompt, options = {}) {
  try {
    ...
    const imageResult = await enhancedAIService.generateCourseImage(prompt, options);
    
    if (imageResult.success && imageResult.data?.url) {
      console.log(`✅ Image generated successfully with ${imageResult.data.provider}`);
      ...
      try {
        ...
        if (imageResult.data.blob) {
          ...
          file = new File([imageResult.data.blob], 'ai-generated-image.png', { type: 'image/png' });
          ...
          if (response.ok) {
            ...
            file = new File([blob], `ai-course-image-${Date.now()}.png`, { type: 'image/png' });
          }
        }
        ...
        if (file) {
          ...
          if (uploadResult?.success && uploadResult.imageUrl) {
            console.log('✅ Image successfully uploaded to S3:', uploadResult.imageUrl);
            return {
              success: true,
              data: {
        ...
        console.warn('📤 S3 upload failed, using original URL:', uploadError.message);
      }
      ...
      if (legacyResult.success && legacyResult.data?.url) {
        ...
        try {
          ...
          if (response.ok) {
            const blob = await response.blob();
            const file = new File([blob], `legacy-ai-image-${Date.now()}.png`, { type: 'image/png' });
            ...
            if (uploadResult?.success && uploadResult.imageUrl) {
              console.log('✅ Legacy AI image uploaded to S3:', uploadResult.imageUrl);
              return {
                success: true,
                data: {
                  ...legacyResult.data,
      ...
      throw new Error(legacyResult.error || 'All image generation methods failed');
    }
 ...
 */
export async function generateMultipleImages(prompt, styles = ['realistic', 'illustration', 'modern']) {
  const results = [];
  
  for (const style of styles) {
    try {
      const styledPrompt = `${prompt}, ${style} style`;
      const result = await generateAndUploadCourseImage(styledPrompt, { style });
      results.push({
        style,
        ...result
      });
