...
Just a clean, realistic, professional photograph-style image with minimal or no text.`;
  ...
  convertImageBlockToHTML(block) {
    ...
    switch (imageLayout) {
      case "image-text":
      case "side-by-side":
        // Handle alignment: 'left' = image left/text right, 'right' = image right/text left
  ...
  convertListBlockToHTML(block) {
    ...
    if (block.variant === "checklist-progress") {
      const completed = Object.values(finalCheckedItems).filter(
        (c) => c,
      ).length;
  ...
  convertInteractiveBlockToHTML(block) {
    try {
      ...
      // First check if data field exists (from blueprint lesson generation)
      if (block.data) {
        interactiveData = block.data;
      }
      // Parse content - handle both string JSON and object
        ...
        interactiveData = block.content || block.data || {};
      }

      const interactiveType =
        block.interactiveType ||
        interactiveData.type ||
        block.metadata?.interactiveType ||
        "quiz";
      const templateId =
        ...
        return `
          <div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-gradient-to-r from-orange-500 to-red-600">
            <div class="interactive-timeline relative">
              <div class="absolute left-4 top-0 bottom-0 w-1 bg-gradient-to-b from-orange-500 to-red-500"></div>
              <div class="pl-12 space-y-6">
                ${timelineData
                  .map(
                    (item, index) => `
                  <div class="relative pb-6">
                    <div class="absolute left-0 top-0 w-8 h-8 bg-white border-4 border-orange-500 rounded-full flex items-center justify-center">
                      <span class="text-orange-500 font-bold text-sm">${index + 1}</span>
                    </div>
                    <div class="bg-gray-50 rounded-lg p-4 border-l-4 border-orange-500">
                      <h4 class="font-semibold text-gray-800 text-lg mb-2">${item.date || item.title || `Step ${index + 1}`}</h4>
                      <p class="text-gray-700 leading-relaxed">${item.description || item.content || item.title || ""}</p>
                    </div>
                  </div>
                `,
                  )
      ...
      return `<div class="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
        <div class="text-gray-700">
          ${typeof block.content === "string" ? block.content : JSON.stringify(block.content || block.data || {})}
        </div>
      </div>`;
    } catch (error) {
...
Format: One outcome per line, each exactly one sentence. Return 4-6 outcomes.`;
      ...
      // Section 3: Key Concepts & Definitions (INTERACTIVE TABS with Images) - 8-12 terms
      // Each term: 1-2 sentences, establishes shared language, removes cognitive friction
      // NEW: Using interactive TABS block instead of table for better engagement
      blocks.push(
        this.createMasterHeading(
...
Focus on terms that are foundational to understanding this lesson. Return 8-12 terms.`;
      ...
      // Create as INTERACTIVE TABS block instead of static table
      const keyTermsList = keyTermsText
        ...
        .filter(Boolean)
        .slice(0, 8); // Limit to 8 tabs for better UX

      if (keyTermsList.length > 0) {
        // Create INTERACTIVE TABS block for key terms with images
        const tabsData = keyTermsList.map((kt, idx) => ({
          title: kt.term,
          content: kt.definition,
          image: null, // Can be populated with AI-generated concept images
        }));

        const keyTermsQualityScore = this.scoreContentQuality(
          keyTermsList.map((kt) => kt.term).join(", "),
          "key-terms",
        );

        blocks.push({
          id: `key-terms-tabs-${Date.now()}`,
          type: "interactive",
          ...
          isAIGenerated: true,
          metadata: {
            variant: "tabs-with-text",
            qualityScore: keyTermsQualityScore,
            description: "Interactive tabs for key concepts with definitions",
          },
        });
      } else {
        blocks.push({
...
Write exactly 350-500 words. Make it detailed, structured, and expert-level.`;
      ...
      // ADD INTERACTIVE LABELED GRAPHIC after deep-dive text (for PREMIUM mode)
      if (options?.contentType === "comprehensive" && options?.includeImages) {
        try {
          // Create labeled graphic hotspots based on key concepts
          const hotspotPrompt = `Based on "${lessonTitle}", create 4-6 key concept labels with brief descriptions (1 sentence each) that could be labeled on a visual diagram or concept map:
Format as "Label - Description" for each concept area.`;

          const hotspotResult = await this.generateWithRefinement(
            hotspotPrompt,
            {
              maxTokens: 300,
              temperature: 0.6,
          ...
          if (hotspots.length > 0) {
            blocks.push({
              id: `labeled-graphic-${Date.now()}`,
              type: "interactive",
              interactiveType: "labeled-graphic",
              data: {
                image: {
                  src: null, // Will be populated with AI-generated concept visualization
                  name: `${lessonTitle} Concept Visualization`,
                },
                hotspots: hotspots,
              },
              order: blockOrder++,
              isAIGenerated: true,
              metadata: {
                variant: "labeled-graphic-with-hotspots",
                description: "Interactive concept map with labeled components",
              },
            });
          }
        } catch (error) {
          console.warn("Failed to create labeled graphic:", error);
        }
      }
...
Format as numbered steps, each 1-3 lines. Return 5-10 steps.`;
      ...
      // For PREMIUM mode with comprehensive content, use INTERACTIVE TIMELINE instead of simple list
      if (options?.contentType === "comprehensive" && stepsItems.length >= 5) {
        // Create TIMELINE interactive block
        const timelineData = stepsItems.slice(0, 8).map((step, idx) => ({
          date: `Step ${idx + 1}`,
          title: step.split("\n")[0] || `Step ${idx + 1}`,
          description: step.split("\n").slice(1).join("\n") || step,
          image: null, // Can add visual representations of each step
        }));

        blocks.push({
          id: `steps-timeline-${Date.now()}`,
          type: "interactive",
          ...
          isAIGenerated: true,
          metadata: {
            qualityScore: stepsQualityScore,
            variant: "timeline-with-steps",
            description: "Interactive timeline for step-by-step process",
          },
        });
      } else {
        // Fallback to regular numbered list for non-comprehensive modes
        blocks.push({
          id: `steps-${Date.now()}`,
          type: "list",
          listType: "numbered",
      ...
      // Section 8: Common Mistakes & Red Flags (INTERACTIVE ACCORDION) - 5-7 items
...
Format: Each mistake in 1-2 lines. Return 5-7 mistakes.`;
      ...
      const mistakesList = mistakesText
        ...
        .filter((mistake) => mistake.length > 0 && mistake.length < 200); // Allow slightly longer for accordion

      // Create INTERACTIVE ACCORDION for mistakes instead of bullet list
      if (mistakesList.length > 0) {
        const accordionData = mistakesList.slice(0, 7).map((mistake, idx) => {
          // Split mistake into title and detail
          const lines = mistake.split("\n");
          const title = lines[0] || `Mistake ${idx + 1}`;
          const content =
            lines.length > 1
              ? lines.slice(1).join("\n")
              : `Learn how to avoid this common pitfall in ${lessonTitle}.`;

          return {
            title: title,
            content: content,
            image: null, // Can add visual examples for mistakes
          };
        });
        ...
        blocks.push({
          id: `mistakes-accordion-${Date.now()}`,
          type: "interactive",
          ...
          isAIGenerated: true,
          metadata: {
            qualityScore: mistakesQualityScore,
            variant: "accordion-with-text",
            description:
              "Collapsible accordion for common mistakes and how to avoid them",
          },
        });
      } else {
        blocks.push({
